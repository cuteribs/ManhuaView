...
export default class Manhuafen extends Parser {
	...
	static getMainTags = async () => {
		try {
			...
			return {
				success: true,
				mainTags: [
					{ title: '精品推荐', items: recommendList },
					{ title: '最新更新', items: updateList },
					{ title: '每月排行', items: rankList },
				],
			};
		} catch (error) {
			console.error(error);
	...
	static getInfo = async (url) => {
		...
		let comic = {
			...
			title: $('div.book-title h1').text().trim(),
			subTitle: $('div.book-cover span.text').text().trim(),
			descr: $('div.book-intro #intro-all').text().trim(),
...
const parseItem2 = ($li) => {
	const item = {
		...
		status: $li.find('.comic_lit_det > p').last().text(),
		author: $li.find('.comic_lit_det > p').first().text().replace('作者：', ''),
	};
	return new Comic(item);
};
...
const parseItem3 = ($li) => {
	const item = {
		...
		status: $li.find('.img_de ul > li').last().text(),
